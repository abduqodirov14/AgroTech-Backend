...
exports.fetchAnalyticsOverview = fetchAnalyticsOverview;
 ...
 */
async function fetchAnalyticsOverview(authenticatedUserId) {
    logger_1.default.info('Retrieving analytics overview', { authenticatedUserId });
    // 1. Fetch user zones
    ...
    const expensesSum = await prismaClient_1.default.transaction.aggregate({
        _sum: { amount: true },
        where: {
    ...
    for (let index = 5; index >= 0; index--) {
        ...
        const salesInMonth = await prismaClient_1.default.productSale.findMany({
            where: {
        ...
        const expensesInMonth = await prismaClient_1.default.transaction.findMany({
            where: {
                userId: authenticatedUserId,
                type: 'EXPENSE',
...
//# sourceMappingURL=analyticsOverviewService.js.map
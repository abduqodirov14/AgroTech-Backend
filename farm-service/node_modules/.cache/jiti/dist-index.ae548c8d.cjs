...
async function loadConfigTsOrJs(configRoot, configFile) {
  const { loadConfig: loadConfigWithC12 } = await Promise.resolve().then(() => jitiImport("c12").then((m) => _interopRequireWildcard(m)));
  const { deepmerge } = await Promise.resolve().then(() => jitiImport("deepmerge-ts").then((m) => _interopRequireWildcard(m)));
  try {
    const {
      config,

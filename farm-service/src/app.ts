...
app.use((req: Request, _res: Response, next: NextFunction) => {
  farmServiceLogger.info(`Incoming request ${req.method} ${req.originalUrl}`);
  next();
});
...
app.get('/health', (_req: Request, res: Response) => {
  res.status(200).json({ status: 'UP', service: 'farm-service' });
});

app.use((error: Error, req: Request, res: Response, _next: NextFunction) => {
  if (error instanceof DomainError) {
    farmServiceLogger.warn(`Domain exception processed: ${error.message}`, {
  ...
  farmServiceLogger.error('Unhandled internal server error', {

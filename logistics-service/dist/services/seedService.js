...
async function seedDemoData(force = false) {
    const count = await prisma_1.default.shipment.count();
    if (count > 0 && !force) {
        return { message: 'Demo data exists', shipments: count };
    }
    if (force) {
        await prisma_1.default.shipmentTracking.deleteMany({});
        await prisma_1.default.shipmentDocument.deleteMany({});
        await prisma_1.default.shipment.deleteMany({});
        await prisma_1.default.vehicle.deleteMany({});
        await prisma_1.default.driver.deleteMany({});
        await prisma_1.default.warehouse.deleteMany({});
        await prisma_1.default.route.deleteMany({});
        await prisma_1.default.logisticsCostSnapshot.deleteMany({});
    }
    const drivers = await Promise.all([
        prisma_1.default.driver.create({ data: { fullName: 'Akmal R.', phone: '+998901111111', telegramId: BigInt(100001), isVerified: true } }),
        prisma_1.default.driver.create({ data: { fullName: 'Bobur R.', phone: '+998902222222', telegramId: BigInt(100002), isVerified: true } }),
        prisma_1.default.driver.create({ data: { fullName: 'Sardor K.', phone: '+998903333333', telegramId: BigInt(100003), isVerified: true } }),
    ]);
    const vehicles = await Promise.all([
        prisma_1.default.vehicle.create({
            data: {
    ...
    const shipments = [
        {
            ...
            status: 'IN_TRANSIT', tempCelsius: 2, progressPercent: 75,
        ...
        },
        {
            ...
            status: 'IN_TRANSIT', progressPercent: 60,
        ...
        },
        {
            ...
            status: 'COMPLETED', tempCelsius: 5, progressPercent: 100,
        ...
        },
        {
            ...
            status: 'PENDING_DRIVER', progressPercent: 0, distanceKm: 400,
        },
    ];
    ...
    const inTransit = await prisma_1.default.shipment.findFirst({ where: { trackId: 'SH-2025-0387' } });
    if (inTransit) {
        await prisma_1.default.shipmentTracking.createMany({
            data: [
    ...
    await prisma_1.default.warehouse.createMany({
        data: [
    ...
    await prisma_1.default.route.createMany({
        data: [
    ...
    await prisma_1.default.logisticsCostSnapshot.create({
        data: {
            period: '2026-06',
            total: 2450,
            fuel: 980,
    ...
    logger_1.logger.info('Logistics demo data seeded');

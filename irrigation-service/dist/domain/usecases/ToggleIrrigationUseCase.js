...
class ToggleIrrigationUseCase {
    ...
    async createSchedule(item) {
        ...
        logger_1.logger.info('📅 Schedule created', item);
        return { success: true, data: item };
    }
    addZone(zone) {
        this.zones.push(zone);
        logger_1.logger.info('➕ Zone added', zone);
    ...
    async uploadSensorReadings(deviceMac, readings) {
        ...
        logger_1.logger.info('📊 Sensor readings updated', { deviceMac, readings });
        ...
        try {
            ...
            if (targetZone) {
                ...
                if (typeof durationMinutes === 'number')
                    targetZone.duration_minutes = durationMinutes;
            }
            const payload = { command, timestamp: new Date().toISOString() };
            if (zoneId)
            ...
            logger_1.logger.info(`✅ ${message}`, payload);
            return {
                success: true,
                message,
            };
